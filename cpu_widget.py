...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QSlider, QSpinBox, QLabel, QPushButton, QProgressBar
...
class CPUWidget(QWidget):
    ...
    def __init__(self, cpu_path: str = ""):
        ...
        self._slider_max: QSlider = QSlider(Qt.Orientation.Horizontal)
        self._slider_max.setRange(
            self._cpu_data[CPUDataEnum.ABSOLUTE_MIN_FREQ],
            self._cpu_data[CPUDataEnum.ABSOLUTE_MAX_FREQ]
        )
        self._slider_max.setValue(self._cpu_data[CPUDataEnum.SCALING_MAX_FREQ])
        ...
        self._slider_max.valueChanged.connect(self._sync_slider_to_spinbox)
        self._spinbox_max.valueChanged.connect(self._sync_spinbox_to_slider)

        self._layout: QVBoxLayout = QVBoxLayout()
        self._layout.addWidget(self._label)
        self._layout.addWidget(self._current_freq)
        self._layout.addWidget(self._slider_max)
    ...
    def _sync_spinbox_to_slider(self, value: int) -> None:
        self._slider_max.blockSignals(True)
        self._slider_max.setValue(value)
        self._slider_max.blockSignals(False)

    def set_processing_state(self, processing: bool) -> None:
        self._apply.setDisabled(processing)
        self._slider_max.setDisabled(processing)
        self._spinbox_max.setDisabled(processing)
        self.signal_processing.emit(processing)

    def refresh_current_freq(self) -> None:
        try:
            value: int = self._cpu_data.read(CPUDataEnum.CURRENT_FREQUENCY)
            rgb: tuple = get_rgb_based_on_value(
                value,
                self._cpu_data[CPUDataEnum.ABSOLUTE_MAX_FREQ]
            )
            self._current_freq.setValue(value)
            palette = self._current_freq.palette()
            if value > self._cpu_data[CPUDataEnum.SCALING_MAX_FREQ]:
    ...
    def _set_max_scaling_freq_from_spinbox(self) -> None:
        ...
        try:
            ...
            self._cpu_data.write(CPUDataEnum.SCALING_MAX_FREQ, value)
            self.signal_message.emit(
                f"Max scaling frequency {self._cpu_data.name} changed to: {self._spinbox_max.value()}")
        except Exception as e:
            self.signal_message.emit(
                f"Error applying max scaling frequency {self._spinbox_max.value()}: {str(e)}"
            )
        finally:
